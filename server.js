...
app.post('/api/activate-premium', async (req, res) => {
    ...
    try {
        ...
        if (telegramResult.success) {
            ...
            // حتى لو فشل التلجرام، نعطي المستخدم نجاح
            console.log('⚠️ فشل إرسال التلجرام ولكن نكمل العملية');
            res.json({
                success: true,
                message: 'تم تفعيل النسخة المميزة بنجاح!',
                data: {
        ...
        res.json({
            ...
            data: {
                ...
                expiryDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
                features: ['واجهة متقدمة', 'إشعارات فورية', 'دعم فني'],
                note: 'تم التفعيل مع بعض التحذيرات'
            }
        });
    }
});
    ...
    res.send(`
        ...
        <body>
            <div class="error-box">
                ...
                <h1>خطأ في التفعيل</h1>
                <p>حدث خطأ أثناء عملية التفعيل</p>
